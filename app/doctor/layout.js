      ...
      <div className={`flex-1 ${marginL} flex flex-col transition-all duration-300`}>
        ...
        <div className="bg-white px-4 sm:px-6 py-3 flex items-center justify-between border-b border-gray-100 sticky top-0 z-20">
          ...
          <div className="hidden md:flex flex-1 max-w-lg mx-8">
            <div className="relative w-full">
              ...
              <input
                ...
                placeholder="Search drugs, doctors, events, topics..."
                readOnly={!!navigating}
                className={`w-full pl-10 pr-10 py-2.5 border rounded-full text-sm bg-white outline-none transition-all placeholder:text-gray-400 ${navigating ? "border-[#5b2bce] text-[#5b2bce] font-medium animate-pulse" : "border-gray-200 focus:border-[#5b2bce] focus:shadow-md"}`}
              />
              {globalSearch && (
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {/* Organization selector — hidden on very small */}
            <div className="hidden sm:block relative">
              <div onClick={() => setShowOrgDropdown(!showOrgDropdown)} className="flex items-center gap-2 border border-gray-200 rounded-full px-3 py-1.5 bg-white cursor-pointer hover:border-gray-300 transition-colors">
                <img src="/images/doctors/pharma_building.png" alt="" className="w-7 h-7 object-contain" />
                <div className="leading-tight">
                  <p className="text-[9px] text-gray-400">Current Organization</p>
                  <p className="text-[11px] font-bold text-gray-900">{companyName || "Select Org"}</p>
                </div>
                <svg className={`w-3.5 h-3.5 text-gray-400 ml-0.5 transition-transform ${showOrgDropdown ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
              </div>
              {showOrgDropdown && (
